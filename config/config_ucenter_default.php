...
define('UC_STANDALONE', 1);				// 獨立模式開關，0=關閉, 1=打開，開啟後將不再依賴UCenter Server。注意：開啟時必須將 UC_CONNECT 改為 mysql ！
// 數據庫相關 (mysql 連接時)
...
define('UC_DBCHARSET', 'utf8mb4');				// UCenter 數據庫字符集
define('UC_DBTABLEPRE', '`ucenter`.uc_');		// UCenter 數據庫表前綴
define('UC_DBCONNECT', '0');				// UCenter 數據庫持久連接 0=關閉, 1=打開
// 頭像相關
define('UC_AVTURL', '');		// 頭像服務的基礎路徑，為空則為默認值，可以設置為獨立域名/路徑（結尾不能有/），配合CDN使用更佳。如涉及 avatar.php 需在其中再配置一次。
define('UC_AVTPATH', '');		// 頭像存儲路徑，為空則為默認值，僅限獨立模式使用，建議保持默認。
